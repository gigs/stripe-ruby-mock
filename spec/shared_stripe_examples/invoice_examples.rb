...
shared_examples "Invoice API" do
  ...
  context "paying an invoice" do
    ...
    it "should work with Stripe::Invoice.pay(invoice_id)" do
      ...
      end.to change { Stripe::Charge.list.data.count }.by 1
  ...
  context "retrieving upcoming invoice" do
    ...
    let(:coupon_amtoff) do
      stripe_helper.create_coupon(id: "100OFF", currency: "usd", amount_off: 100_00, duration: "repeating",
        duration_in_months: 6)
    end
    let(:coupon_pctoff) do
      stripe_helper.create_coupon(id: "50OFF", currency: "usd", percent_off: 50, amount_off: nil, duration: "repeating",
        duration_in_months: 6)
    end
    let(:product) { stripe_helper.create_product(id: "prod_123") }
    let(:plan) do
      stripe_helper.create_plan(id: "50m", product: product.id, amount: 50_00, interval: "month", nickname: "50m",
        currency: "usd")
    end
    let(:quantity) { 3 }
    ...
    before(with_discount_amtoff: true) do
      customer.coupon = coupon_amtoff.id
      customer.save
    end
    before(with_discount_pctoff: true) do
      customer.coupon = coupon_pctoff.id
      customer.save
    end
    ...
    describe "parameter validation" do
      it "fails without a subscription or subscription plan if subscription proration date is specified", live: true do
        expect do
          Stripe::Invoice.upcoming(customer: customer.id, subscription_proration_date: Time.now.to_i)
      ...
      it "fails without a subscription if proration date is specified", live: true, with_subscription: true do
        expect do
          Stripe::Invoice.upcoming(customer: customer.id, subscription_plan: plan.id,
    ...
    describe "proration" do
      ...
      [false, true].each do |with_trial|
        describe "prorating a subscription with a new plan, with_trial: #{with_trial}" do
          let(:new_monthly_plan) do
            stripe_helper.create_plan(id: "100m", product: product.id, amount: 100_00, interval: "month")
          end
          let(:new_yearly_plan) do
            stripe_helper.create_plan(id: "100y", product: product.id, amount: 100_00, interval: "year")
          end
          ...
          it "prorates while maintaining billing interval", live: true do
            ...
            customer.save
            query = {customer: customer.id, subscription: subscription.id, subscription_plan: new_monthly_plan.id,
                     subscription_proration_date: proration_date.to_i, subscription_quantity: new_quantity}
            ...
            expect(upcoming.lines.data.last.amount).to eq with_trial ? 0 : 20_000
          ...
          it "prorates while changing billing intervals", live: true do
            ...
            customer.account_balance = -credit_balance
            customer.save
            query = {customer: customer.id, subscription: subscription.id, subscription_plan: new_yearly_plan.id,
                     subscription_proration_date: proration_date.to_i, subscription_quantity: new_quantity}
            query[:subscription_trial_end] = (DateTime.now >> 1).to_time.to_i if with_trial
            ...
            expect(upcoming.lines.data[1].amount).to eq with_trial ? 0 : 20_000
            expect(upcoming.lines.data[1].quantity).to eq new_quantity
          end
    ...
    it "sets the start and end of billing periods correctly when plan has an interval_count" do
      ...
      expect(@upcoming.period_start + 6_652_800).to eq(@upcoming.period_end) # 6652800 = +11 weeks
      expect(@upcoming.period_end).to eq(@upcoming.lines.data[0].period.start)
      expect(@upcoming.period_end + 6_652_800).to eq(@upcoming.lines.data[0].period.end) # 6652800 = +11 weeks
      expect(@upcoming.next_payment_attempt).to eq(@upcoming.period_end + 3600) # +1 hour
    end
    ...
    it "chooses the most recent of multiple subscriptions" do
      ...
      expect(@upcoming.period_start + 604_800).to eq(@upcoming.period_end) # 604800 = 1 week
      expect(@upcoming.period_end + 604_800).to eq(@upcoming.lines.data[0].period.end) # 604800 = 1 week
    ...
    context "calculates month and year offsets correctly" do
      it "for one month plan on the 1st" do
        ...
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2014, 1, 1, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
      ...
      it "for one year plan on the 1st" do
        ...
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2012, 1, 1, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
      ...
      it "for one month plan on the 31st" do
        @plan = stripe_helper.create_plan(product: product.id, id: "one_mo_plan")
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2014, 1, 31, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
      ...
      it "for one year plan on feb. 29th" do
        ...
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2012, 2, 29, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
      ...
      it "for two month plan on dec. 31st" do
        ...
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2013, 12, 31, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
      ...
      it "for three month plan on nov. 30th" do
        ...
        @sub = Stripe::Subscription.create(plan: @plan.id, customer: customer.id,
          current_period_start: Time.utc(2013, 11, 30, 12).to_i)
        @upcoming = Stripe::Invoice.upcoming(customer: customer.id)
